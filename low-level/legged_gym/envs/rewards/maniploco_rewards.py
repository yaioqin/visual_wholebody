...
class ManipLoco_rewards:
    ...
    def _reward_pfg_feasible(self):
        """Physical-feasibility reward from a full virtual IK solve.

        Failure must return zero.  The 0.2 lower bound applies only after IK
        succeeds; applying it before the feasibility mask would reward
        unreachable torso states.
        """
        cfg = self.env.cfg.rewards.pfg
        current_arm_q = self.env.dof_pos[:, self.env.arm_dof_indices_tensor]
        joint_error = torch.sum(
            torch.abs(self.env.pfg_q_ideal - current_arm_q),
            dim=-1,
        )
        shaped_reward = torch.exp(-cfg.joint_error_scale * joint_error)
        shaped_reward = torch.clamp(
            shaped_reward,
            min=cfg.minimum_feasible_reward,
            max=1.0,
        )
        reward = torch.where(
            self.env.pfg_ik_success,
            shaped_reward,
            torch.zeros_like(shaped_reward),
        )
        # reset_idx divides accumulated metrics by episode duration in seconds.
        # Multiplying by dt makes the logged value a time-averaged IK success
        # rate in [0, 1] for full-length episodes.
        metric = self.env.pfg_ik_success.float() * self.env.dt
        return reward, metric
    # <<< PFG REWARD PATCH (3d_m_a2b) <<<

    def _reward_low_manipulability(self):
        cfg = self.env.cfg.rewards.manipulability

        arm_jacobian = self.env.get_arm_jacobian(refresh=False)
        singular_values = torch.linalg.svdvals(arm_jacobian)

        log_eta = torch.sum(
            torch.log(torch.clamp(singular_values, min=cfg.eps)),
            dim=-1,
        )
        ...
        soft_ratio = float(cfg.soft_ratio)          # 0.20
        critical_ratio = float(cfg.critical_ratio)  # 0.10

        # 0.20 -> 0.10：惩罚从 0 平滑增加到 0.3
        soft_progress = torch.clamp(
            (soft_ratio - eta_ratio)
            / (soft_ratio - critical_ratio),
            min=0.0,
            max=1.0,
        )
        soft_cost = 0.3 * torch.square(soft_progress)

        # 低于 0.10：额外惩罚从 0 增加到 0.7
        critical_progress = torch.clamp(
            (critical_ratio - eta_ratio) / critical_ratio,
            min=0.0,
            max=1.0,
        )
        critical_cost = 0.7 * torch.square(critical_progress)

        cost = soft_cost + critical_cost
        metric = eta_ratio * self.env.dt
